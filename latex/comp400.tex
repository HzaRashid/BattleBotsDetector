...
flexibility (allows multiple data types in its feature set), and its proven efficacy in bot detection \citep{ilias2023multimodal},
...
that, through a sigmoid activation \(\sigma \) on the projected modalities, 
learns how much each one contributes to the final fused representation for downstream classification.
This example is for the reader’s intuition; in practice many codebases depart from these equations.  
Arevalo \emph{et al.}’s own demo\footnote{\url{https://github.com/johnarevalo/gmu-mmimdb/blob/master/model.py}}  
omits separate projections for each modality and applies the nonlinearity directly to the inputs.  
In our implementation, the hidden states are concatenated for weighting 
for improved training stability, and \( \mathbf{W}_z \) projects 
onto two dimensions to impose a bottleneck and speed up training. 
...
% the hidden states are concatenated for weighting, which improved training stability,
% and \( \mathbf{W}_z \) projected onto two dimensions to
