...
The output is used (with possibly further transformations)
...
In practice, implementations tend to depart\footnote{Arevalo \emph{et al.}’s own demo \url{https://github.com/johnarevalo/gmu-mmimdb/blob/master/model.py}; omits separate projections for each modality and applies the nonlinearity directly to the inputs.}
...
Resembling the work of \emph{Ilias et al.}, our intial efforts with the GMU used the following modalities:
\begin{itemize}
...
To improve the initial GMU, we developed two refined variants:

\begin{itemize}
    \item \textbf{GMUFusionWithNorm:}  
    Introduced modality-specific projections and \textit{LayerNorm} to balance their scales. 
    Normalization was applied after fusion to stabilize the representation.

    \item \textbf{GMUFusionWithGlobalSkip:}  
    Added a global residual connection that projected raw modality embeddings directly to the hidden space, and applied modality-specific dropout.
\end{itemize}
...
This addition improved performance, supporting our intuition 
that a dynamic temporal feature can help capture richer distinctions 
...
Given these concerns, we transitioned to cross-modal attention mechanisms, 
hypothesizing that they could acheive nontrivial performance with fewer architectural tweaks.


\subsubsection{Cross-Attention}
Equipped with three modalities (user description, content-DNA, and time-DNA), it 
would have been computationally expensive to generalize the work of \emph{Ilias et al.}, 
which would require six multihead-attention layers to consider 
each cross-modal interaction in both directions.
A natural alternative is to employ a single self-attention 
block where each token corresponds to one of the modalities. 
We found little success with this approach, but given that it plays
...
\begin{itemize}
    \item Apply a simple GMU to the DNA modalities to create an intermediate representation 
    of fused-DNA.
...
essentially computing scaled dot-product attention \citep{vaswani2017attention}
\begin{align}
...
A mean pooling is applied to both attention layers' outputs to form 
...
\paragraph{Session 16 - Description, Content, Time}
...
\begin{itemize}
    \item \textbf{Modality Preparation:}
    \begin{itemize}
        \item The user description embedding is projected into a hidden space.
        \item Content and time DNA sequences are fused via the GMU as stated before.
    \end{itemize}
    ...
    \begin{itemize}
        \item The two cross‑attention outputs are concatenated into a tiny sequence and fed through a single position‑wise FFN, enabling direct interaction between modalities while sharing parameters for better regularization.
