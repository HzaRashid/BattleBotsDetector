...
temporal posting behaviour, showing how bots can easily manipulate features 
to appear human-like under stationary models.
% when viewed through static temporal models.
% Since bots can easily manipulate features 
% to appear human-like under stationary models, 
Thus, we motivate the use of a dynamic encoding scheme,
and consider various multimodal approaches with an emphasis on DNA-inspired 
behaviour modeling.
...
For session 11, we extracted embeddings (all-MiniLM-L6-v2) for each user, 
and applied LDA topic modeling under the assumption that bots target 
...
remove noise from the LDA modeling. Finally, a balanced Random Forest with 100 trees 
was trained and evaluated using stratified 80/20 train/test splits.
...
\subsubsection{Session 12 – Mean Embedding + BERTopic + Temporal Statistics}
...
Result:
The use of dynamic topic modeling and behavioral 
signals significantly improved classification performance.
...
% The KS-based analysis demonstrates that static temporal 
...
% these static summaries cannot capture the complex timing behaviors exhibited by sophisticated bots.
% that static temporal 
% features are not reliable indicators of bot-like activity
...
\textbf{Strengths:}
\begin{itemize}
  \item \textit{Hierarchical fusion}: In addition to the bidirectional cross‑attention steps, the GMU’s gated combination of content and time embeddings produces richer joint representations than simple concatenation alone.
...
\textbf{Conclusion:} While the session 16 model introduced a promising fusion 
paradigm combining gated mechanisms and cross-modal attention, 
...
\begin{itemize}
    % \item \textit{Marginal Utility of Emoji Features:} Despite their inclusion, the emoji embeddings did not noticeably improve performance. 
    % Their sparse and high-dimensional nature may have diluted their utility. 
    ...
    \item \textit{Noise \& Training Instability:} The emoji embeddings did not improve performance
    and their sparse and high-dimensional nature may have introduced noise to the model. This would also explain the training instability, 
    as the model exhibited greater sensitivity to hyperparameter configurations. 
...
Overall, the inclusion of emoji DNA did not yield the expected results 
due to the added noise and training instability.
The experiment revealed the challenges 
